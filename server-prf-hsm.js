...
import hsmOps from "./hsm-secure-operations.js";
import cloudHsm from "./hsm/cloudhsm-session.js";
import { secureWipe } from "./hsm/hsm-utils.js";
...
app.post("/setup-complete", async (req, res) => {
  ...
  try {
    ...
    const hsmResult = await hsmOps.deriveKeyInHSM(prfBytes, wallet_id);
    ...
    console.log(`   Private key: Secured in HSM (non-extractable)`);
...
app.post("/sign-with-prf", async (req, res) => {
  ...
  try {
    ...
    const hsmKey = await hsmOps.deriveKeyInHSM(prfBytes, wallet_id);
    const publicKey = hsmKey.publicKey;
    const pubkeyHex = bytesToHex(publicKey);

    console.log(`✅ Derived key for signing wallet ${wallet_id} (HSM secured)`);
    console.log(`   Public key: ${pubkeyHex.slice(0, 16)}...`);
    console.log(`   Private key: NEVER EXPOSED (secured in HSM)`);
    ...
    // Perform signing entirely in HSM
    const det = await hsmOps.performMuSig2SignInHSM(
      prfBytes,
      wallet_id,
      clientNonces,
      sortedKeys,
...
if (!process.env.VERCEL) {
  ...
  // Initialize HSM before starting server
  async function startServer() {
    try {
      // Initialize HSM master key
      await hsmOps.initializeMasterKey();

      // Test security model
      console.log("\n🔒 Testing HSM security model...");
      const testPRF = Buffer.from("test-prf-value");
      const securityTest = await hsmOps.testSecurityModel(
        testPRF,
        "test-wallet",
      );

      if (!securityTest.secure) {
        console.error(
          "❌ SECURITY FAILURE: PRF can derive keys without master key!",
        );
        process.exit(1);
      }
      ...
      // Start server
      cloudHsm.attestLog("server.start", {
        port: PORT,
      ...
      const shutdown = async (signal) => {
        ...
        try {
          await cloudHsm.shutdown();
        } catch (shutdownError) {
          console.error("Error during CloudHSM shutdown:", shutdownError);
        } finally {
          process.exit(0);
        }
      };
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
      ...
      console.error("Failed to start HSM server:", error);
      process.exit(1);
    }
  }

  // Start the server with HSM initialization
  startServer();
}
